    ...
    def run(self):
        while self._running:
            if not self._paused:
                ...
                # 1. Query Miner Data
                try:
                    resp_stats = self.session.get(f"{self.miner_url}/cgi-bin/stats.cgi", timeout=2.0)
                    raw_stats = resp_stats.json()

                    resp_pools = self.session.get(f"{self.miner_url}/cgi-bin/pools.cgi", timeout=2.0)
    ...
    def _on_data_received(self, data: Dict[str, Any]):
        ...
        if self.cutoff_triggered:
            ...
            if is_over_stop:
                ...
                # Temperature safe: reset warning countdown
                self.overheat_wrn_seconds = 0
                self.cutoff_alert_label.hide()
